...
/* export*/ function activate() {
  var index1 = {
    ...
    action: "`action` keys define text for actions such as milking cows, dismounting horses, etc.",
    advMode: "*I'm unsure exactly what these do, but they appear to be related to commands*",
    apple: "`apple` keys only appear when playing on iOS devices. They relate to connecting to iCloud or LAN.",
    attribute: "`attribute` keys relate to entity attributes",
    attribution: "`attribution` has only two keys, one of them being `attribution.goBack=Go Back`. The other has a link to https://minecraft.net/attritbution ",
    authentication: "`authentication` keys are used for logging in and purchasing (EE)",
    ...
    build: "`build` texts control the messages shown when players try to violate the build limit",
    chalkboardScreen: "`chalkboard` contains button labels for a discontinued EE block",
    chat: "`chat` keys contain text for chat settings, game messages, etc.",
    chestScreen: "`chestScreen` keys control inventory labels in chests and large chests",
    chooseRealmScreen: "`choosRealmScreen` controls text in Realms menu in Play menu",
    customTemplatesScreen: "`customTemplatesScreen` has one key, which controls templates",
    ...
    codeScreen: "`codeScreen` appears to set controls for a code editor, but I have no idea which one",
    color: "`color` controls display names of colors",
    commandBlockScreen: "`commandBlockScreen` controls text in the command block menu",
    seargeSays: "I have no idea what the heck `seargeSays` is for man",
    commandBlock: "`commandBlock` controls the default name of a command block in chat (`@`), as well as it's `genericName` (`Command Block`)",
    commands: "`commands` contains descriptions of commands shown in chat autocomplete, as well as success and error messages",
    connect: "`connect` controls messages shown when connecting to a server",
    connect_gamepad: "`connect_gamepad` appears to be used for text whown when connecting controllers",
    gamepad_disconnect: "`gamepad_disconnect` seems to control text shown when a control unexpectedly disconnects",
    container: "`container` controlls labels of interactive blocks (e.g. beacon, hopper, stonecutter, etc.) when interacting",
    controller: "`controller` texts control action labels in inventory and modifying keybinds",
    course: "`course` is used by EE courses, whatever those are",
    crafting: "I have never seen the messages in `crafting` in normal gameplay, but they seem to be error messages",
    createWorld: "`createWorld` texts are used for creating highly customized worlds",
    createWorldUpsell: "Exactly what it sounds like",
    createWorldScreen: "`createWorldScreen` contains labels for contorls in creating and editing world settings",
    crossPlatformToggle: "`crossPlatformToggle` controls settings menu for controlling cross-platform play",
    realmsPendingInvitationScreen: "These texts are used when viewing pending realm invites.",
    realmsInvitationScreen: "Controls menu for inviting players to realms",
    realmJoining: "`realmJoining.progressTitle` is the message shown when joining a realm",
    realmsClearMembers: "`realmsClearMembers` is used when clearing members from a realm",
    realmsSharingScreen: "Fairly self-explanatory",
    realmsCreateScreen: "`realmsCreateScreen` texts are used when creating a realm",
    ...
    localWorld: "`localWorld` texts are used when user has no single-player worlds",
    ...
    dayOneExperience: "These keys contain text shown to players when they first download the game",
    ...
    difficulty: "`difficutly.lock` texts contain prompts for locking a world's difficulty",
    dimension: "`dimension` contains the names of the 3 dimensions",
    disabledSkin: "for prompts given to players when they are using a server-disabled skin",
    disconnect: "`disconnect` texts contain the messages given when disconnected from a server",
    disconnectionScreen: "`disconnectionScreen` contains the errors shown when trying to connect to a server",
    storageSpaceWarningScreen: "`storageSpaceWarningScreen` contains the messages shown when you run out of storage",
    edu: "For EE keys",
    ...
    addExternalServerScreen: "These texts are shown when adding a server to your server list",
    ...
    comment: "`comment` texts are shown in some kind of comments section? idek :/",
    furnaceScreen: "`furnaceScreen` texts are used for labels in furnace menu",
    gameArgument: "`gameArgument.featureUnsupported` = This version of Minecraft doesn't support this feature",
    gameMode: "`gameMode` contains names of gamemodes and game mode update message",
    gameTip: "For hints",
    generator: "`generator` seems to relate to custom world generation settings (old, amplified, etc.)",
    globalPauseScreen: "`globalPauseScreen` seems to contain text shown in the global pause screen",
    ...
    level: "`level` apperas to contain texts shown when importing/exporting a world",
    ...
    itemGroup: "`itemGroup` contains the names of groups in the creative inventory",
    jigsaw: "For displays in the jigsaw block. See https://minecraft.fandom.com/wiki/Jigsaw_Block ",
    joincode: "EDU joincode",
    key: "`key` contains names of keybinds (e.g. Jump, Inventory, etc.",
    keyboard: "`keyboard` contains names of keys. (e.g. ESCAPE, RETURN, A, 1, F7, etc.)",
    lanServer: "For texts relating to LAN worlds",
    licensed_content: "`licensed_content.viewLicensedContent` has a link to https://minecraft.net/licensed-content/ ",
    ...
    notification: "`notification` contains the display messages of items with lock_in_inventory [components](https://minecraft.fandom.com/wiki/Item_components)",
    ...
    patchNotes: "`patchNotes` contains the default messages shown in an update popup",
    ...
    portfolioScreen: "For use with the deprecated [camera](https://minecraft.fandom.com/wiki/Camera)",
    ...
    resourcePack: "`resourcePack` texts are shown in rp menu or when adding a resource pack\n\nAlso other resource pack related featuers",
    ...
    raycasting: "`raycasting` contains the errors shown when user does not have a rtx-compatible device",
    ...
    seedPicker: "`seedPicker` contains the texts shown when picking a preset seed",
    selectServer: "Texts shown on the server tab under the play menu",
    selectWorld: "`selectWorld` contains labels for Worlds menu and edit world menu",
    selectTemplate: "`selectTemplate` contains the texts shown when creating a new world from template",
    ...
    soundCategory: "`soundCategory` contains generic sound type categories. See vanilla files",
    ...
    stream: "`stream` contains texts seen when streaming through a connected twitch account",
    profanity_filter: "`profanity_filter` appears to contain labels for a profanity filter",
    terms_and_conditions: "`terms_and_conditions.viewTermsAndConditions` contains a link to https://minecraft.net/terms ",
    ...
    worldConversionErrorPrompt: "`worldConversionErrorPrompt` contains error messages that occur when converting a world",
    worldError: "`worldError` contains error messages that occur when trying to open a world",
    userData: "`userData` texts contain error messages that occur when recovering user data",
    ...
    verification: "`verification` contains error messages that occur when MC can't verify user license",
    ...
    packupgradewarning: "`packupgradewarning` seems to have warnings in case of invalid `.mcpack` import attempt",
    pack: "`pack` contains default texts for authors of packs",
    manifestvalidation: "`manifestvalidation` has texts for the status messages for resource and behavior packs",
    uiPackError: "`uiPackError` contains several resource pack errors",
    jsonValidationError: "`jsonValidationError` contains several JSON parse errors",
    serverUI: "`serverUI` contains errors for creating forms",
    content: "`content` messages pertain to importing packs and worlds",
    worldTemplate: "`worldTemplate` contains names of packs which are built in (Festive Mash-up 2016, Greek Mythology Mash-up, etc.)",
    eduTemplateWorld: "Similar to `worldTemplate` but for EE",
    worldConversionComplete: "`worldConversionComplete` appears to contain texts for converting worlds",
    tips: '`tips` contains the text that is shown when a world is loading. For example, "Do people actually read these?"',
    splitscreen: "`splitscreen` contains messages relating to players joining a world in splitscreen mode",
    ...
    contentlog: "`contentlog` contains labels for buttons in content log settings",
    client: "`client` has update names for 1.0-1.5 and 1.11. I don't know what they are there for.",
    cloud_upload: "`cloud_upload` seems to have support messages for uploading a world to Mojang Player Support",
    dr: "`dr` contains marketplace texts",
    onlinePlay: "`onlinePlay` contains the messages warning players that online play is not rated",
    copyCoordinates: "`copyCoordinates` contains texts shown in copy coordinate UI",
    ...
    clientscript: "`clientscript` contains messages relating to gametest debug messages",
    world_recovery: "`world_recovery` contains messages shown when recovering worlds",
